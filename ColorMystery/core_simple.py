...
• Numéros dans chaque zone (sauf blancs), police réduite si mini-zone
• Palette lisible : texte blanc sur couleurs sombres, noir sinon
• Blancs (fond + vrais blancs) exclus de la palette
• Numérotation commence à 1 sans trou
...
# ─── presets de simplification ──────────────────────────────────────────
# low : aucun flou pour traits nets sur vectoriel
SIMPLIFY_PRESET = {
...
# ─── quantisation + fusion de micro-zones ─────────────────────────────
def quantize(img: np.ndarray, k: int, min_area: int):
    h, w = img.shape[:2]
...
# ─── génération de la planche complète ────────────────────────────────
def generate_number_sheet(image: Image.Image, k: int=16, simplify: str="medium") -> Image.Image:
    ...
    # 3) déterminer labels blancs à sauter
    skip = set()
    for lbl in unique:
        r,g,b = palette[lbl]
    ...
    for r in regionprops(lab_conn):
        lbl = labels[r.coords[0][0], r.coords[0][1]]
        if lbl in skip or r.area<50:
            continue
        mask = lab_conn==r.label
        font = FONT if r.area>=100 else SMALL_FONT
        place_number(mask, str(mapping[lbl]), font)

    # 7) palette sous l'image
    strip_h = 60
    ...
    for i,col in enumerate(palette_ordered):
        ...
        dp.rectangle([x,y,x+40,y+40], fill=rgb)
        lum = 0.2126*rgb[0] + 0.7152*rgb[1] + 0.0722*rgb[2]
        tc = "white" if lum<150 else "black"
        dp.text((x+20,y+20), str(i+1), fill=tc, anchor="mm", font=FONT)
...
def main():
    ...
    diff = {"easy":8,"medium":16,"hard":24}[args.difficulty]
    ...
    sheet = generate_number_sheet(img, diff, args.simplify)
